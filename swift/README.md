...
- マウスで選択したテキストの近くからオンデマンド処理やプリセットを実行する
...
- `Resources/default-presets.json`: 初回起動・設定リセット時に使うデフォルトプリセット

## デフォルトプリセットを編集する

`Resources/default-presets.json` の配列を直接編集します。各プリセットでは次の項目を指定できます。

- `name`: 設定画面と選択メニューに表示する名前
- `shortcutKey`: `Ctrl+1` などのグローバルショートカット
- `prompt`: LLMへ渡す処理指示
- `enabled`: 起動時に有効にするかどうか

`id` は省略可能です。デフォルトプリセットは最大5件で、初回起動または「デフォルトに戻す」を実行したときに読み込まれます。すでに保存されているユーザー設定は自動的には上書きしません。
